...
<body class="bg-[#FDFDFC] dark:bg-[#0a0a0a] text-[#1b1b18] p-6 min-h-screen">
    <div class="max-w-2xl mx-auto bg-white dark:bg-[#161615] p-6 rounded shadow">
        <div class="flex items-center justify-between mb-4">
            <h1 class="text-2xl font-medium">Edit Todo</h1>
        ...
        <form action="{{ route('todos.update', $todo) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input id="title" name="title" value="{{ old('title', $todo->title) }}" type="text" required
            ...
            <div class="mb-4">
                ...
                <textarea id="description" name="description" rows="4"
                    class="w-full px-3 py-2 border border-[#e3e3e0] rounded focus:outline-none focus:ring">{{ old('description', $todo->description) }}</textarea>
            </div>

            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
                <div>
                    <label class="block text-sm font-medium mb-1" for="category">Category</label>
                    <input id="category" name="category" value="{{ old('category', $todo->category) }}" type="text"
                        class="w-full px-3 py-2 border border-[#e3e3e0] rounded focus:outline-none focus:ring" />
                </div>
                ...
                <div>
                    ...
                    <input id="due_date" name="due_date"
                        value="{{ old('due_date', optional($todo->due_date)->format('Y-m-d') ?? $todo->due_date) }}"
                        type="date" class="w-full px-3 py-2 border border-[#e3e3e0] rounded" />
                </div>
            </div>

            <div class="flex items-center gap-3">
                <button type="submit" class="px-4 py-2 bg-[#1b1b18] text-white rounded-sm">Save changes</button>
